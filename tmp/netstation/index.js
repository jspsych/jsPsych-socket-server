...
function _cat(...strings) {
    ...
    let fmt = args.map(a => `s`).join('');
    return jsPack(fmt, ...args);
}
...
class _DataFormat {
    constructor() {
        ...
        this._translationTable[myTypeof(true)] = ['boolean', '=?'];
        this._translationTable[myTypeof(1)] = ['long', '=l'];
        this._translationTable[myTypeof(1.1)] = ['double', '!d'];
        this._translationTable[myTypeof('')] = ['TEXT', 's'];
    }

    _pack_data(data) {
        let hints = this._translationTable[myTypeof(data)];
    ...
    _pack_dict(table, pad = false) {
        ...
        let nkeysStr = jsPack('=B', keys.length);
    ...
    pack(key, timestamp = null, label = null, description = null, table = null, pad = false) {
        ...
        */
        ...
        let labelStr = _pstring(label), descriptionStr = _pstring(description), tableStr;
